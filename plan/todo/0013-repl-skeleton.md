...
an async callback that processes each user prompt. The actual agent wiring happens in 0014;
...
- Slash commands stubbed:
  ...
  - `/tools` -- placeholder, prints `(no tools registered)` until 0014 wires it.
  - `/reset` -- placeholder, prints `(no history to reset)` until 0014 wires it.
- The slash commands' real data (tool list, history clear) come in 0014 via additional
