...
The queue up to here fixes the blockers, corrects the docs, cuts `0.2.0-rc.3` (0129), and gathers
live runtime evidence (0130). Then it stopped. Nothing joined those two into a final release, and
the two tasks nearest the end actively contradict each other if read as the end: 0126 was written
to produce final `[0.2.0]` documentation, while 0129 explicitly cuts an RC, says it is not final,
...
test, and deliberately separate from 0129 so that a second RC -- if rc.3 exposes another
public-surface correction -- costs one more pass through 0129 rather than a rewrite of the final.
...
- **0130's evidence exists** for both architectures and is green, including 0114's live tier.
- **0129's recorded soak parameters are satisfied** -- the window, the qualifying use, the
  evidence source, and the blocker policy, all decided in 0129 before rc.3 shipped. This task
  checks that record; it does not write it. An RC nobody installed has not been
  integration-tested, however many days elapsed.
- **No open defect of the class 0129's record calls blocking.**

If any fails, the outcome is a new RC through 0129, and this task waits.
...
- **Final package validation**, re-run rather than assumed to still hold: 0125's snapshot gate,
  0129's version guard, 0129's install-shaped check, and the combined
...
- **Hard: 0129** -- there is no final without the RC it cuts.
- **Hard: 0130** -- the runtime evidence is an exit criterion.
...
- `plan/todo/0129-cut-0.2.0-rc.3.md` -- the RC, and the loop this task returns to on failure.
