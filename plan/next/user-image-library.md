...
- **Indexing** at config load: scan `<root>/*/image.toml`. The directory name is the
  ...
  `ConfigSource` is the project directory (`plan/next/config-path-provenance.md`).
...
- **Hard: `plan/next/config-path-provenance.md`.** A library project's `dockerfile` and `context`
  are relative to the project directory. Until an image-config carries its own base directory,
  every path in the index resolves against the repo root.
- **Soft: `plan/next/e2e-imageconfig-sidecars-bitrot.md`.** The e2e feature does not compile
  today, so any end-to-end acceptance here needs that fix first.
