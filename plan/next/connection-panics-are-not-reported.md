...
- `crates/outrig/src/network.rs`, the accept loop's final drain: `while conns.join_next().await.is_some() {}`
...
This predates 0117 in effect -- on trunk, connections were `tokio::spawn`ed with the handle
dropped, so a connection panic was already lost -- but 0117 added a reporting contract for the
