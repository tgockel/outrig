...
> enforcement *and* this test;
> `plan/done/phase/0002-sidecars/tasks/0002-48-enforce-the-public-api-snapshot.md` delivered the
> first and formally deferred this one, with the reasoning in its `## Decisions`. 0.2.0's release
...
0002-16 settled that `container`, `image`, `mcp_proxy`, and `network` are supported public API, not
...
So the half of the surface that 0002-16 promoted to a contract has no test defining that contract.
...
caller. The proxy's dispatch tests are no longer even out-of-crate: 0002-18 sealed `BackingClient`
...
- Construct the spec types through the constructors 0002-17 shipped (`ContainerWorkspace::new`,
...
- Build `ImageTag` through `ImageTag::new` and read it through `as_str` / `into_string`; 0002-18
  made the tuple field private, so the `ImageTag(name)` form `cococlaw` used is gone.
- Drive `create_initialized` through `ContainerCreateOptions`, which is the one entry point whose
  shape 0002-18 changed.
...
`plan/done/phase/0002-sidecars/tasks/0002-18-options-structs-and-sealing.md`.
