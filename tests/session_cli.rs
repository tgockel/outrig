//! Integration tests for `outrig::cli::ls`, `outrig::cli::logs`, and
//! `outrig::cli::discard`. Each test owns its own tempdir as the session
...
use outrig::cli::{discard, logs, ls};
...
async fn logs_substring_resolves_uniquely() {
    ...
    let (dir, _) = outrig::cli::resolve_session_arg(&store, "1344").expect("substring");
...
async fn logs_substring_ambiguous_errors() {
    ...
    let err =
        outrig::cli::resolve_session_arg(&store, "20260501").expect_err("ambiguous should error");
    let msg = format!("{err}");
