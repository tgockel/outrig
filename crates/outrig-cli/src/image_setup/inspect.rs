//! `outrig image inspect` -- read a local image's OutRig labels.
...
/// Inspect a local image's declared OutRig config from OCI labels. This is
/// metadata-only: no pull, no container start, and no MCP server initialization.
pub async fn run(image_ref: &str) -> Result<()> {
    ...
    let labels = image::read_image_labels(&tag, None).await?;
