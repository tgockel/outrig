...
fn spec_to_toml_value(spec: &McpServerSpec) -> toml_edit::Value {
    ...
    // The table arm reads through the accessors rather than destructuring
    // `Full`, so it needs no dead catch-all and borrows instead of cloning.
    // Each key's elision still has to track the matching `skip_serializing_if`
    // on the spec; a field added to the variant needs one more `if` here.
    match spec {
        ...
        spec => {
            ...
            if let Some(sidecar) = spec.sidecar() {
                table.insert("sidecar", sidecar.into());
            }
            if let Some(image) = spec.image() {
                table.insert("image", image.into());
            }
            let args = spec.args();
            if !args.is_empty() {
...
mod tests {
    ...
    fn show_merged_serializes_view_primary_and_elides_default() {
        let with_view = McpServerSpec::entrypoint("docker.io/mcp/filesystem:latest")
            .with_args(["/workspace"])
            .with_view(outrig::config::SidecarView::Primary);
        ...
        let default_view = McpServerSpec::entrypoint("img");
