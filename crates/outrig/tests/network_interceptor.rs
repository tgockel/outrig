...
/// Runs `nft` in the container's network namespace, the way the interceptor
/// does. Returns the exit status rather than asserting on it: the point of
/// some of these is that the command fails.
fn nft_in_netns(pid: u32, args: &[&str]) -> Output {
    Command::new("nsenter")
        .args(["-t", &pid.to_string(), "-U", "-n", "nft"])
        .args(args)
        .output()
        .expect("spawn nft")
}

/// The name of the table interception installed, from the namespace itself.
fn outrig_table_name(pid: u32) -> Option<String> {
    let listed = nft_in_netns(pid, &["list", "tables"]);
    String::from_utf8_lossy(&listed.stdout)
        .lines()
        .find_map(|line| Some(line.split_whitespace().nth(2)?.to_string()))
        .filter(|name| name.starts_with("outrig_"))
}
...
/// delete-and-recreate by anything else holding `NET_ADMIN` in the namespace
/// leaves the replacement standing, and detach says it could not finish
/// rather than taking someone else's rules with it.
///
/// Needs no traffic, which is why it is here rather than among the record
/// tests: what it exercises is the identity the undo carries.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn detach_leaves_a_table_that_replaced_the_one_it_created() {
    let _guard = E2E_LOCK.lock().await;
    common::init_tracing();
    ...
    let table = outrig_table_name(pid).expect("attach installs a redirect table");

    // Someone else in the namespace takes the name: what outrig created is
    // gone, and a different table answers to its name from here on.
    let deleted = nft_in_netns(pid, &["delete", "table", "inet", &table]);
    assert!(deleted.status.success(), "{deleted:?}");
    let recreated = nft_in_netns(pid, &["create", "table", "inet", &table]);
    assert!(recreated.status.success(), "{recreated:?}");

    let detached = interceptor.detach(container.name()).await;
    assert!(
        detached.is_err(),
        "a table this attach did not create is not its to delete, and that is \
         a teardown that did not finish"
    );
    assert!(
        netns_has_outrig_table(pid),
        "the replacement must still be there: detach deletes by the handle it \
         was given, which this table does not have"
    );

    let cleaned = nft_in_netns(pid, &["delete", "table", "inet", &table]);
    assert!(cleaned.status.success(), "{cleaned:?}");
    interceptor.shutdown().await.expect("shutdown");
    container
        .stop(Duration::from_secs(2))
        .await
        .expect("stop container");
